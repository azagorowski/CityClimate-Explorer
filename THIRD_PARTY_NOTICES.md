...
City metadata, coordinates, country relationships, population, QIDs, and sitelinks are queried from Wikidata. Wikidata structured data is released under **CC0 1.0**, which permits commercial reuse. The application still attributes Wikidata for transparency.
...
`data/preloaded/country_capitals.json` is local startup metadata and includes English Wikipedia source links where available. Runtime climate caches preserve source name, source language, page title, source URL, and whether the result was English-primary, native-language fallback, or Wikidata fallback. Do not remove those fields when exporting or redistributing cached climate results.
