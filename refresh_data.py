...
def main() -> None:
    ...
    parser.add_argument("--min-population", type=int, default=DEFAULT_POPULATION_THRESHOLD, help="Population threshold.")
    parser.add_argument("--force", action="store_true", help="Bypass local caches and refetch remote sources.")
    args = parser.parse_args()

    logging.basicConfig(level=logging.INFO, format="%(levelname)s:%(name)s:%(message)s")
    cities = fetch_cities(limit=args.limit, min_population=args.min_population, force_refresh=args.force)
